...
function unIndexIndexedGeometry( geometry ) {
	...
	var newUvs = [];
	if( doUvs)newUvs = new TypedArrayHelper( oldFaces.length * 3, 3, THREE.Vector2, Float32Array, 2, XY );
	var v, w;
...
( function () {
	...
	THREE.BufferSubdivisionModifier.prototype.smooth = function ( geometry ) {
		...
		*******************************************************/
		...
		geometry.addAttribute( 'position', new THREE.BufferAttribute( newVertices.buffer, 3 ) );
		if( doUvs )geometry.addAttribute( 'uv', new THREE.BufferAttribute( newUVs.buffer, 2 ) );
